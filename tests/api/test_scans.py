...
TestClient against the real app (real policies, real in-memory store —
no mocking).
"""
...
def test_post_scans_on_clean_repo_returns_complete_with_no_findings(tmp_path):
...
def test_post_scans_on_empty_repo_returns_one_finding_per_policy(tmp_path):
    response = client.post("/api/v1/scans", json={"repo_path": str(tmp_path)})
...
def test_post_scans_respects_policy_subset(tmp_path):
...
def test_post_scans_with_nonexistent_repo_path_returns_400():
    response = client.post(
...
def test_get_scan_after_post_returns_the_same_result(tmp_path):
    _make_clean_repo(tmp_path)
...
def test_get_scan_with_unknown_id_returns_404():
