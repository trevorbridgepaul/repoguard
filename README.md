...
- `GET /api/v1/scans/{scan_id}` — **requires auth** — poll for a previously submitted scan's result
...
- Scans aren't scoped per-user yet — any authenticated user can see any `scan_id`
